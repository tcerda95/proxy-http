...
public class HttpBodyParserFactory {
	...
	public HttpBodyParser getServerHttpBodyParser (HttpResponseParser headersParser, Method method)
		throws ParserFormatException {
	...
	private HttpBodyParser buildServerBodyParser (HttpHeaderParser headersParser)
		throws ParserFormatException {
		HttpBodyParser parser = buildBodyParser(headersParser);
	...
	private boolean hasChunked (HttpHeaderParser headersParser) {
		return headersParser.hasHeaderValue(Header.TRANSFER_ENCODING) && BytesUtils
			.equalsBytes(headersParser.getHeaderValue(Header.TRANSFER_ENCODING),
				HeaderValue.CHUNKED.getValue(), BytesUtils.TO_LOWERCASE);
	}

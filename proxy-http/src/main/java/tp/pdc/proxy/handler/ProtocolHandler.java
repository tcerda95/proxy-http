...
public class ProtocolHandler implements Handler {
	...
	private void process (SelectionKey key) {
		...
		if (parser.hasFinished() || clientClosedRead) {
			...
			LOGGER
				.debug("Registering protocol handler for write and read: parsed but not finished");
		...
		try {
			...
			if (parser.hasFinished() || clientClosedRead || unrecuperableError) {
				if (!writeBuffer.hasRemaining()) {
					LOGGER.debug(
						"Closing connection to protocol client: parser finished and no bytes left in writeBuffer");
					trySocketClose(socketChannel);
				} else {
					LOGGER.debug(
						"Registering protocol client for write: parser finished but not all bytes sent");
					key.interestOps(SelectionKey.OP_WRITE);
				}
			} else {
				if (!writeBuffer.hasRemaining()) {
					LOGGER.debug(
						"Registering protocol client for read: all bytes sent but parser not finished");
					key.interestOps(SelectionKey.OP_READ);
				} else {
					LOGGER.debug(
						"Registering protocol client for read and write: not all bytes sent and parser not finished");

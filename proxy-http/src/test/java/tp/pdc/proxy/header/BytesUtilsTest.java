...
public class BytesUtilsTest {
	...
	public void lengthPut () {
		...
		ByteBuffer inputBuffer = ByteBuffer
			.wrap("hola como estas hoy?".getBytes(ProxyProperties.getInstance().getCharset()));

		BytesUtils.lengthPut(inputBuffer, outputBuffer, 4);
		outputBuffer.flip();
		assertEquals("hola",
			new String(outputBuffer.array(), outputBuffer.position(), outputBuffer.remaining(),
				ProxyProperties.getInstance().getCharset()));
	}
